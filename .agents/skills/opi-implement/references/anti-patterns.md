...
| Never remove a worktree with dirty or uncontained ledger state | Cleanup must prove the canonical ledger is clean, no temp remains, and required checkpoints reached the destination branch. Historical incidents belong in snapshots and Git history, not the active rule. |
...
| Never feed a v1 snapshot into a live v2 plan | Frozen v1 snapshots are audit-readable history, not migration input. Live plan and execution paths accept v2 only. |
| Never add unregistered design/plan docs, snapshot files, `CLAUDE.md`, `AGENTS.md`, or skill source to `spec_files` | Only reviewed Phase delivery source files listed in `SKILL.md` are normative alongside the parent spec. Arbitrary process docs and skill files create circular reinit failures. |
| Never execute bundled independent deliverables as one monolithic task | One commit, one DoD, and one evaluator cannot truthfully close independent slices. Split the registered source item during graph admission and review the decomposition as a unit. |
...
| Never turn a registered Phase Non-Goal or handoff into executable work | Delivery sources use Non-Goals and handoffs to preserve scope. New authorization requires human shaping, source registration, and a new plan review. |
...
| Never satisfy a phase by implementing its Non-Goals | Phase designs use Non-Goals to preserve product scope. Any excluded capability requires a separate reviewed source rather than opportunistic implementation. |
