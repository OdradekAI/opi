...
| `tasks[].verification` | object | const | Tier-specific gate spec. `library_gates` contains exactly one authoritative D.1 smoke command; `smoke_addendum` is `null` or one gap-only command string for D.0/D.3 artifact, generator, Python-contract, or authoritative-platform evidence. Scenario commands remain in `acceptance_scenarios[].verification`. |
