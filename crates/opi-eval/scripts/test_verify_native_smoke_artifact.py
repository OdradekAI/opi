#!/usr/bin/env python3
"""Behavioral tests for crates/opi-eval/scripts/verify-native-smoke-artifact.py.
...
native-smoke definition of done requires. These tests build a synthetic but
...
def trial_receipt(trial: str, product: str, task: str, group: str,
    ...
    native: dict = {
        "schema": "opi-eval-trial-receipt/1",
        "id": trial,
...
def build_trial(stage: Path, cfg: str, trial: str, product: str, task: str,
    ...
    write_json(bundle / "intent.json",
               {"schema": "opi-eval-run-bundle-intent/1", "trial": trial})
    ...
    write_json(root / "report.json",
               {"schema": "opi-eval-benchmark-report/1", "task": task,
...
def build_stage(tmp: Path, candidate: str) -> tuple[Path, Path, dict]:
    ...
    dispatch = {
        ...
        "bound_scripts": {
            "producer": {"path": "crates/opi-eval/scripts/native-smoke.sh",
                         "role": "producer", "sha256": sha(b"producer\n")},
            "agent-builder": {"path": "crates/opi-eval/scripts/build-agent-artifacts.sh",
                              "role": "builder", "sha256": sha(b"builder\n")},
            "provider": {"path": "crates/opi-eval/scripts/scripted-provider.py",
                         "role": "provider", "sha256": sha(b"provider\n")},
            "verifier": {"path": "crates/opi-eval/scripts/verify-native-smoke-ci.py",
    ...
    markers = ["__OPI_EVAL_CANARY_openssl_selfsigned_cert__",
               "__OPI_EVAL_CANARY_batched_eval_parity__",
               "__OPI_EVAL_CANARY_abs_module_cache_flags__"]
    ...
    material_json = {
        ...
        "provider": {
            "script": {"path": "crates/opi-eval/scripts/scripted-provider.py",
        ...
        "agents": {
            product: {
                ...
                "model": "scripted:eval" if product == "opi"
                         else "scripted:scripted/eval",
    ...
    write_json(material / "materialize-receipt.json",
               {"schema": "opi-eval-materialize-receipt/1", "wrappers": {}})
    ...
    for bench, short in BENCHMARKS.items():
        ...
        write_json(root / "run-report.json", {
            "schema": "opi-eval-run-report/1",
            "experiment": f"opi-eval-native-{short}",
    ...
    provider_log.write_text(
        "".join(json.dumps({"schema": "opi-eval-scripted-provider-log/1",
                            "request_sha256": sha(f"req-{i}".encode()),
                            "script": "opi-eval-scripted-provider/1"}) + "\n"
...
def seal(stage: Path, tmp: Path, expires_in_days: int = 90) -> dict:
    ...
    write_json(seal_out / "artifact-manifest.json",
               {"schema": "opi-eval-native-artifact-manifest/1", "files": manifest})
    outer = {
...
def build_git_repo(tmp: Path) -> tuple[Path, str]:
    ...
    bodies = {
        "crates/opi-eval/scripts/native-smoke.sh": b"producer\n",
        "crates/opi-eval/scripts/build-agent-artifacts.sh": b"builder\n",
...
class ArtifactVerifier(unittest.TestCase):
    ...
    def test_bare_tar_accepted(self) -> None:
        result = self.verify(
            "EVAL-A02", artifact=self.sealed["tar"])
        self.assertEqual(result.returncode, 0, result.stderr)
