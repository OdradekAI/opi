...
| Current implementation | `opi` 0.6.4 workspace, Phase 8 runtime stabilization complete; Phase 7 reliability/observability and Phase 5 package/process-adapter surfaces are present |
...
Status: substrate implemented in the current `0.6.4` workspace.
...
Status: implemented in the current `0.6.4` workspace.
...
Status: complete in the current `0.6.4` workspace.
...
Status: planned; recast from the previous Phase 12.
...
|---|---|
| Provider OAuth / subscription auth | `Models/Auth` seam is stable; credential store, redaction, doctor, session interaction, login UX, refresh, and revocation are designed. |
