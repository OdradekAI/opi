...
| P17-AUT-008 | A tool excluded by effective policy **MUST NOT** appear in the provider-facing tool schema for that request; projection **MUST** be recomputed from trusted registrations for every provider request. | Reference Product policy owner | Consecutive-request tool-projection snapshot tests. |
