#!/usr/bin/env bash
# Phase 18 exact agent build/locator script (task 18.14).
#
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --out) OUT=$2; shift 2 ;;
    *) echo "phase18-build-agent-artifacts: unknown argument: $1" >&2; exit 2 ;;
  esac
done
if [ -z "$OPI_SOURCE" ] || [ -z "$PI_SOURCE" ] || [ -z "$OUT" ]; then
  echo "phase18-build-agent-artifacts: --opi-source, --pi-source, and --out are required" >&2
  exit 2
fi
...
# --- Opi -----------------------------------------------------------------
echo "phase18-build-agent-artifacts: building opi (locked release)" >&2
...
cargo_v = run(["cargo", "-V"])
identity = {
    "schema": "phase18-agent-identity/1",
    "agent": "opi",
...
# --- pi ------------------------------------------------------------------
echo "phase18-build-agent-artifacts: building pi (locked source build)" >&2
...
echo "phase18-build-agent-artifacts: identities written to $OUT" >&2
