#!/usr/bin/env python3
"""Contract tests for the Phase 18 materialization artifact verifier.
...
SCRIPT = Path(__file__).with_name("verify-phase18-materialization-artifact.py")
...
class Workspace:
    ...
    def __init__(self, root: Path | None = None) -> None:
        self.root = root or Path(tempfile.mkdtemp(prefix="phase18-artifact-verify-"))
...
class RejectsReceiptContract(unittest.TestCase):
    ...
    def test_wrong_schema_or_identity_rejects(self) -> None:
        for field, value in (
            ("schema", "phase18-materialization-receipt/2"),
            ("lock_id", "phase18-other"),
...
class RejectsWorkflowBinding(unittest.TestCase):
    ...
    def test_qualified_github_workflow_ref_normalizes(self) -> None:
        ...
        receipt["workflow"]["ref"] = (
            "OdradekAI/opi/.github/workflows/phase18-lock-materialization.yml@refs/heads/main"
        )
...
class AcceptsAndWritesLock(unittest.TestCase):
    def test_seeded_workspace_writes_expected_lock(self) -> None:
        ...
        self.assertEqual(result.returncode, 0, output(result))
        lock = json.loads(lock_path.read_text(encoding="utf-8"))
        self.assertEqual(lock["schema"], "phase18-external-lock/resolved/1")
        self.assertEqual(lock["lock_id"], "phase18-linux-x86_64")
        self.assertEqual(lock["platform"], "linux-x86_64")
        self.assertEqual(lock["resolved_by_task"], "18.3")
        ...
        future_ids = {slot["id"]: slot["owner_task"] for slot in lock["future"]}
        self.assertEqual(
            future_ids,
            {slot["id"]: slot["owner_task"] for slot in declared if slot["owner_task"] != "18.3"},
        )
        self.assertEqual(
            {slot["id"] for slot in lock["resolved"]},
            {slot["id"] for slot in declared if slot["owner_task"] == "18.3"},
        )
...
class AcceptsRealRepositoryPins(unittest.TestCase):
    def test_real_repository_static_pins_audit(self) -> None:
        ws = Workspace(root=Path(tempfile.mkdtemp(prefix="phase18-real-root-")))
