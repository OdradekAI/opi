#!/usr/bin/env python3
"""Static verifier for the Phase 18 native-smoke producer contract (18.14).
...
def verify_provider(text: str, f: Findings) -> None:
    if 'SCHEMA = "phase18-scripted-provider/1"' not in text:
...
def main() -> int:
    ...
    if findings.rows:
        for row in findings.rows:
            print(row)
        return 1
    print("phase18-native-ci: producer contract verified "
